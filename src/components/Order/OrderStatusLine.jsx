        ...
        {statusKeys.map((status, index) => {
          ...
          return (
            <div key={status} className="text-center" style={{ width: `${100 / statusKeys.length}%` }}>
              {/* Circle */}
              <div
                className={`rounded-circle mx-auto mb-1 bg-${isActive ? statusInfo.badge_color : 'secondary'}`}
                style={{
                  width: '24px',
                  height: '24px',
                  lineHeight: '24px',
              ...
              {/* Label */}
              <small className={`d-block ${isActive ? `text-${statusInfo.badge_color}` : 'text-muted'}`}>
                {statusInfo.name}
              </small>
            </div>
          )
